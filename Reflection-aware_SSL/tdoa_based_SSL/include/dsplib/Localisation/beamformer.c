...
void beamformerFindMaxima(struct objBeamformer *myBeamformer, struct objPreprocessor *myPreprocessor, struct objPotentialSources *mySources)
{
    ...
    ***************************************************************************/
    ...
    for (indexMic = 0; indexMic < myBeamformer->myMicrophones->nMics; indexMic++)
    {
        ...
        if(maxFrequencyPower < myPreprocessor->micArray[indexMic]->xMaxPower)
        {
            maxFrequency = (float)(myPreprocessor->micArray[indexMic]->xMaxPowerIndex);
            maxFrequencyPower = myPreprocessor->micArray[indexMic]->xMaxPower;
        }

        //averageFrequency += (float)(myPreprocessor->micArray[indexMic]->xMaxPowerIndex);
    }

    //averageFrequency /= myBeamformer->myMicrophones->nMics;

    //mySources->frequency = averageFrequency;
    mySources->frequency = maxFrequency;
    ...
    ***************************************************************************/
    ...
    for (indexPotential = 0; indexPotential < myBeamformer->BF_MAXSOURCES; indexPotential++)
    {
        ...
        mySources->sourcesEnergy[indexPotential] = energy;
